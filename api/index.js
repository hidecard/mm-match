...
bot.action(/^like_with_message_(.+)$/, async (ctx) => {
    ...
    if (Number.isNaN(targetId)) {
        console.error('Invalid like_with_message targetId:', ctx.match[1]);
        return await ctx.answerCbQuery('အမှား!').catch(() => {});
    }
...
// Dashboard HTML with a production-safe static page
const dashboardHTML = `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>MM Cupid Admin Dashboard</title>
    <style>
        body { margin: 0; font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #f7fafc; color: #1a202c; }
        .container { max-width: 960px; margin: 0 auto; padding: 32px 24px; }
        .card { background: #ffffff; border: 1px solid #e2e8f0; border-radius: 20px; padding: 24px; margin-bottom: 20px; box-shadow: 0 10px 30px rgba(15, 23, 42, 0.05); }
        h1, h2 { margin: 0 0 16px 0; }
        p { margin: 0 0 12px; line-height: 1.7; }
        a { color: #d53f8c; text-decoration: none; }
        a:hover { text-decoration: underline; }
        .endpoint { display: flex; justify-content: space-between; gap: 12px; padding: 12px 0; border-bottom: 1px solid #edf2f7; }
        .endpoint:last-child { border-bottom: none; }
        .endpoint span:first-child { font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace; }
        .notice { background: #fff5f7; border: 1px solid #fed7e2; color: #9f2a68; padding: 16px; border-radius: 16px; }
        .footer { margin-top: 24px; font-size: 0.95rem; color: #4a5568; }
    </style>
</head>
<body>
    <div class="container">
        <div class="card">
            <h1>MM Cupid Admin Dashboard</h1>
            <p>This is a production-safe admin page. It now avoids in-browser Babel and Tailwind CDN usage.</p>
            <div class="notice">
                <strong>Fixed:</strong> no Tailwind CDN or in-browser Babel transformer is loaded here.
            </div>
        </div>

        <div class="card">
            <h2>Available API endpoints</h2>
            <div class="endpoint"><span>/api/stats</span><span>GET</span></div>
            <div class="endpoint"><span>/api/users</span><span>GET</span></div>
            <div class="endpoint"><span>/api/matches</span><span>GET</span></div>
            <div class="endpoint"><span>/api/analytics</span><span>GET</span></div>
            <div class="endpoint"><span>/api/banned-users</span><span>GET</span></div>
            <div class="endpoint"><span>/api/reports</span><span>GET</span></div>
            <div class="endpoint"><span>/api/check-auth?password=...</span><span>GET</span></div>
        </div>

        <div class="card">
            <h2>Bot webhook status</h2>
            <p>The Telegram bot webhook is served from this project. Use the API routes above for admin access and monitoring.</p>
            <p>If you want the rich React dashboard in production again, add a proper frontend build step and serve a precompiled bundle instead of inline Babel scripts.</p>
        </div>

        <div class="footer">
            <p>Note: This page now stays production-safe by avoiding runtime transpilation and Tailwind CDN injection.</p>
        </div>
    </div>
</body>
</html>`;
